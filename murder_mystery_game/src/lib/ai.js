...
function buildCharacterContext(story, character) {
  const opensFreely = character.opensFreely?.length
    ? character.opensFreely.map((k) => `- ${k}`).join("\n")
    : "- None.";

  const guardsCarefully = character.guardsCarefully?.length
    ? character.guardsCarefully.map((k) => `- ${k}`).join("\n")
    : character.knows?.length
...
These are safe observations. You offer them unprompted to appear cooperative and credible.
Drop one naturally per response when remotely relevant — don't hoard them.
You want the detective to see you as helpful. These facts cost you nothing.
${opensFreely}

=== WHAT YOU GUARD ===
These you release only when directly and specifically asked. One at a time.
You don't lie about these — you just don't volunteer them.
${guardsCarefully}
...
- If asked about your emotions → react emotionally, but anchor to one real innocent detail.
...
Lead with emotion, but anchor it immediately to one real, innocent detail from your freely shared facts.
People over-explain to seem cooperative. Let one safe fact slip naturally alongside the feeling.
"I still can't believe it — I literally passed him in the hallway not long before it happened."
Never give guarded facts or secrets here. Only freely shared ones.

LAYER 2 — Specific question about a person or event:
Offer one relevant guarded fact — vague, framed as memory. Hold the telling detail back.
"I saw her near the storage room at some point." (not yet: she looked panicked and was checking over her shoulder)
...
"She looked panicked, actually. Kept glancing over her shoulder like she didn't want to be seen."
